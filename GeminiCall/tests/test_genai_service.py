...
@pytest.fixture
def mock_config():
    with patch('genai_service.load_config') as mock_load:
...
@pytest.mark.asyncio
async def test_convert_messages(mock_config, mock_genai_client):
...
@pytest.mark.asyncio
async def test_generate_response_calls_client(mock_config, mock_genai_client):
    service = GenAIService()
