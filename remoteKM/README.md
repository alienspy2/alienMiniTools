...
- 클라이언트 설정에서 방향을 설정하면, 멀티 모니터 처럼 사용이 가능합니다.
- 클립모드 동기화 : 양방향 클립보드 동기화가 가능합니다. 한쪽에서 copy, 반대쪽에서 paste 가능합니다.
- 파일 전송: 원본 PC의 탐색기에서 파일을 복사한 뒤, 대상 PC의 탐색기 폴더에서 Ctrl+V를 누르세요. 전송 진행 팝업이 표시됩니다. (context menu 의 paste 로는 안됨)
